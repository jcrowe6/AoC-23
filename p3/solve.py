...
# given a location of a symbol in the engine, looks at all adjactent squares and processes the numbers
...
for i in range(len(engine)):
    for j in range(len(engine[0])):
        if engine[i][j] != '.' and engine[i][j] not in digits: # symbol! look for numbers around
